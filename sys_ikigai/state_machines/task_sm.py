...
from sys_ikigai.state_machines._sm_base import StateMachine, Transition
...
def task_state_machine() -> StateMachine:
    ...
    sm.add_transition(
        Transition("blocked", "in_progress", "unblock", audit_message="Task unblocked")
    )
    ...
    sm.add_transition(
        Transition("in_progress", "cancelled", "cancel", audit_message="Task cancelled")
    )
