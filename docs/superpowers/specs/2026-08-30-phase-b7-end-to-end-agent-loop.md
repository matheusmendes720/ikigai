...
- Acompanhamento de SONHOS → OBJETIVOS → PROJETOS → DELIVERABLES (camadas hierárquicas)
- Feedback estocástico encadeado pelo PAE
- Context retrieval retroativo por níveis de registro (planned vs realized)
- Burndown / expectativa vs realidade em A2UI

**Acceptance:**
- ADR committed
- Each algorithm component has explicit OUT OF BACKEND SCOPE verdict (replaces DEFER/SHIP/PARTIAL taxonomy)
- 6 open questions enumerated but reframed as prompt-template concerns (B8+ work, not B7)
- Cross-references all relevant memories including [[algorithm-scope-reframed-2026-08-30]]
