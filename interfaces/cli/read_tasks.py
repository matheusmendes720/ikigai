...
def list(
    horizon: str | None = typer.Option(
        ...
        None, "--done", "-d", help="Filter by done status"
    ),
...
def done(
    task_id: str = typer.Argument(
        ..., help="Task ID (first 8 chars of the record uuid)"
    ),
) -> None:
...
# W5.3 — kill switch consumer UX sub-app
from interfaces.cli.kill_switch import app as kill_switch_app
...
def show_mesh(ueid: str) -> dict:
    ...
    # Detect mismatches
    statuses = [
        v.get("status") for v in view.values() if isinstance(v, dict) and "status" in v
    ]
    mismatches = []
...
def plan_list(
    ...
    if all_forks:
        ...
        if json_out:
            ...
            console.print(table)
            mismatches_flat = [
                m for j in joined for m in j.get("_mesh", {}).get("mismatches", [])
            ]
            if mismatches_flat:
...
# === Phase 9: Deep Agent task filter ===


def _is_deep_agent_task(task: dict) -> bool:
    """Return True if task was generated by the Deep Agent (provenance present)."""
    return task.get("source_fork") == "deep_agent" or bool(task.get("agent_id"))


@app.command(name="deep-agent-tasks")
def deep_agent_tasks(
    json_output: bool = typer.Option(
        False, "--json", help="Machine-readable JSON output"
    ),
    limit: int = typer.Option(50, "--limit", "-n", help="Max tasks to show"),
) -> None:
    """Show tasks written by the Deep Agent (source_fork=deep_agent or agent_id present)."""
    all_tasks = _read_tasks()
    da_tasks = [t for t in all_tasks if _is_deep_agent_task(t)]

    if json_output:
        console.print_json(json.dumps(da_tasks[:limit], default=str))
        return

    if not da_tasks:
        console.print("[dim]No Deep Agent tasks found.[/dim]")
        return

    table = Table(title=f"Deep Agent Tasks — {len(da_tasks)} total")
    table.add_column("UEID", style="cyan", width=36)
    table.add_column("Title", style="white")
    table.add_column("Priority", style="yellow", width=9)
    table.add_column("Status", style="magenta", width=10)
    table.add_column("Source", style="green", width=14)
    table.add_column("mtime", style="dim", width=10)

    for t in da_tasks[:limit]:
        ueid = str(t.get("ueid", "?"))[:36]
        title = (t.get("title", "?") or "?")[:48]
        prio = t.get("priority", "medium")
        done_mark = "done" if t.get("done") else "pending"
        source = t.get("source_fork", "") or t.get("agent_id", "-") or "-"
        mtime = t.get("mtime", "") or t.get("created_at", "")
        if mtime and len(mtime) > 10:
            mtime = mtime[:10]
        table.add_row(ueid, title, prio, done_mark, source, mtime)

    console.print(table)
    console.print(f"\n[dim]{len(da_tasks)} Deep Agent tasks total[/dim]")
