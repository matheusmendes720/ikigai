...
def handle(args: dict) -> dict:
    ...
    store = _store()
    src = store.load(
        inp.from_snapshot_id
    )  # raises FileNotFoundError → upstream JSON-RPC error
...
def _field_diff(src: dict, dst: dict) -> list[TuiboardChange]:
    ...
    for k in all_keys:
        ...
        if s_val != d_val:
            diffs.append(
                TuiboardChange(ueid=src["ueid"], field=k, before=s_val, after=d_val)
            )
    return diffs
