"""render_score_course_observation — course vector (0-100) via LLM prompt.
...
## Tarefa
Estime a pontuação de curso/educação (course, 0-100) com base no ciclo atual.
Justifique em 2-3 frases em pt-BR.

Retorne JSON: {{"course_score": <int 0-100>, "rationale": "<pt-BR 2-3 sentences>"}}""",
)


def render_score_course_observation(state: dict[str, Any]) -> dict[str, Any]:
    """Render course vector observation via LLM (or deterministic stub in fake mode)."""
    if os.environ.get("IKIGAI_FAKE_LLM", "0") == "1":
        return {"course_score": 40, "rationale": "[FAKE-LLM stub for test]"}
