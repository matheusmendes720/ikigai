"""render_heuristics_regime_observation — regime state via LLM prompt.
...
## Tarefa
Determine o estado de regime atual: PUSH, MAINTAIN, REDUCE ou RECOVER.
Justifique em 2-3 frases em pt-BR.

Retorne JSON: {{"regime": "<PUSH|MAINTAIN|REDUCE|RECOVER>", "rationale": "<pt-BR 2-3 sentences>"}}""",
)


def render_heuristics_regime_observation(state: dict[str, Any]) -> dict[str, Any]:
    """Render regime observation via LLM (or deterministic stub in fake mode)."""
    if os.environ.get("IKIGAI_FAKE_LLM", "0") == "1":
        return {"regime": "MAINTAIN", "rationale": "[FAKE-LLM stub for test]"}
