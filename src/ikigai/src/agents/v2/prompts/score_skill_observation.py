"""render_score_skill_observation — skill vector (0-100) via LLM prompt.
...
## Tarefa
Estime a pontuação de habilidade (skill, 0-100) com base no ciclo atual.
Justifique em 2-3 frases em pt-BR.

Retorne JSON: {{"skill_score": <int 0-100>, "rationale": "<pt-BR 2-3 sentences>"}}""",
)


def render_score_skill_observation(state: dict[str, Any]) -> dict[str, Any]:
    """Render skill vector observation via LLM (or deterministic stub in fake mode)."""
