"""render_h5_skill_velocity — H5 skill velocity observation via LLM prompt.
...
## Tarefa
Estime a velocidade de skill (0.0-1.0) com base no ciclo atual.
Justifique em 2-3 frases em pt-BR.

Retorne JSON: {{"h5_skill_velocity": <float 0.0-1.0>, "rationale": "<pt-BR 2-3 sentences>"}}""",
)


def render_h5_skill_velocity(state: dict[str, Any]) -> dict[str, Any]:
    """Render H5 skill velocity observation via LLM (or deterministic stub in fake mode)."""
    if os.environ.get("IKIGAI_FAKE_LLM", "0") == "1":
        return {"h5_skill_velocity": 0.4, "rationale": "[FAKE-LLM stub for test]"}
