"""render_h3_regime_fsm — H3 regime FSM transition observation via LLM prompt.
...
## Tarefa
Determine a transição de regime FSM recomendada: PUSH, MAINTAIN, REDUCE ou RECOVER.
Justifique em 2-3 frases em pt-BR.

Retorne JSON: {{"h3_regime": "<PUSH|MAINTAIN|REDUCE|RECOVER>", "rationale": "<pt-BR 2-3 sentences>"}}""",
)


def render_h3_regime_fsm(state: dict[str, Any]) -> dict[str, Any]:
    """Render H3 regime FSM observation via LLM (or deterministic stub in fake mode)."""
