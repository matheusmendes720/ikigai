...
def get_taskdog_tasks() -> list[dict[str, Any]]:
    ...
    result = run_taskdog(
        ["list", "--fields", "id,name,status,priority,tags,deadline,estimated_duration,note"]
    )
    ...
    for line in lines[4:]:
        ...
        parts = [
            p.strip() for p in raw_parts[1:-1]
        ]  # Skip first and last which are empty from border chars
...
def analyze_vault_taskdog_gaps() -> dict[str, Any]:
    ...
    actionable_deliverables = [
        d for d in deliverables if d.get("status") in ("IN_PROGRESS", "TODO")
    ]
    ...
    missing_tasks = []
    process_tracker = next(
        (d for d in deliverables if "process-tracker" in d.get("slug", "")), None
    )
        ...
        if blocker_data:
            has_b1_task = any(
                (
                    (
                        "b1" in t["name"].lower()
                        or "blocker" in t["name"].lower()
                        or "h3" in t["name"].lower()
                    )
    ...
    if objective:
        has_demo_task = any(
            "demo" in t["name"].lower() and "portfolio" in t["name"].lower() for t in ikigai_tasks
        )
...
def sync_vault_to_taskdog(dry_run: bool = False) -> dict[str, Any]:
    ...
    for missing in analysis["missing_tasks"]:
        if dry_run:
            print(
                f"🟢 [DRY-RUN] Would create: {missing['title']} (p={missing['priority']}, tags={missing.get('tags', [])})"
            )
        else:
...
""")
        for missing in analysis["missing_tasks"]:
            print(
                f"      + {missing['title']} (priority={missing['priority']}, tags={missing.get('tags', [])})"
            )
