...
- **Phase 8.x IKIGAI agent plumbing** (2026-09-08, all SHIPPED):
  ...
  - **8.7** (`b63e801`): **CLI wired** — `v2 daily/weekly` commands added to `interfaces/cli/v2.py` via `interfaces/cli/_v2_skills.py` (`load_skill_manifest` + `invoke_skill` + `ensure_mcp_server_bound`); 4 CliRunner + FakeMcpServer tests pass
  - **8.8** (`011dc20`): **monthly/quarterly** + entry-point validation — `monthly`/`quarterly` CLI commands; 4 skills × 2 parametrized tests = 8 + 3 plain = 11 new tests
  - **8.9** (`89a324a`): **LangGraph Studio integration** — `Makefile:test` expanded (2 new pytest lines for v2 dispatch); `.env.example` documents runtime env vars; `interfaces/cli/tests/conftest.py` comment-only fix; `--confcutdir=.` Makefile flag prevents dual-module identity conflict
- **Phase 9.x night-shift arc** (2026-09-09, all SHIPPED — autonomous loop, $0 LLM cost):
  - **9.0** (`21e2a58`): **stale v2 interface test references** — `src/ikigai/tests/test_v2_interface_dispatch.py` (13 tests) + `test_v2_e2e_smoke.py` (6 tests) updated to use Phase 8.7-8.8 revived daily/weekly/monthly/quarterly skill surface (no more ModuleNotFoundError on deleted v2 symbols)
  - **9.1** (`020ad029`): **CI/CD integration** — `.github/workflows/ci.yml` extended (+77 lines): `quality-gates` matrix gains `v2-nodes` entry running 9 test paths with `-m "not integration"`; NEW `v2-nodes-smoke` top-level job uses `--confcutdir=.` + `IKIGAI_FAKE_LLM=1` for fast feedback. CI jobs list: `[code-review-checks, quality-gates, mcp-gateway-contract, review-queue-worker-contract, v2-nodes-smoke]`
  - **9.2** (`533fa85`): **documentation refresh** — NEW `docs/PHASE_8X_USAGE.md` (212 lines, 7KB, 17 sections: Quick Start, Skill Manifests, CLI Reference for daily/weekly/monthly/quarterly, LangGraph Studio, CI Matrix, Troubleshooting 4-subtopics, See Also); `docs/LANGRAPH_DEV.md` header updated to link to new doc
  - **9.3** (`a239afe`): **operator TUI Skills tab** — 6th tab "Skills" (key `6`) added to `interfaces/tui/operator/`. `data.py` gains `SkillRow` dataclass + `load_skill_rows()`. TUI remains observer-only per CLAUDE.md dual-layer architecture
  - **9.4** (`dbff4aa8`): **PAV test rename + .gitignore** — `git mv src/ikigai/tests/test_v2_pav_intentions.py test_v2_surface_intentions.py` per [[archived-feature-not-vocabulary-2026-09-06]]; 3 test functions renamed; `.gitignore` adds 5 patterns (`0`, `14`, `agent('*`, `int`, `None`) for 0-byte bash-redirect artifacts per CLAUDE.md
  - **9.5** (`fc88963`): **approval gate for tag_and_persist** — per Phase 8.4 plan decision (e). `tag_and_persist_node` checks `state.get("approval_state") == "approved"` BEFORE _write call (matches `proposal_executor.py:113` pattern); 2 new tests + 2 updated fixtures; PermissionError raised if not approved
...
- **Remaining for "complete app":**
  - **9.x real LLM deployment** (PROPOSED): wire `langgraph deploy` + Anthropic API for production inference. Needs `ANTHROPIC_API_KEY` + user adjudication on token spend. CLAUDE.md "Zero LLM in pipeline" rule + [[algorithm-gate-dropped-2026-09-03]] make this gated
  - **10.x data mesh v1.2** (DEFERRED, gated on user adjudication): `update/delete/done` actions; was DROPPED 2026-09-03 per [[algorithm-gate-dropped-2026-09-03]]
  - **Pre-existing test hardening** (OUT OF SCOPE per [[archived-feature-not-vocabulary-2026-09-06]]): 5 PAV-archived test files + 68 fail / 9 error budget in full-repo regression
- **Memory:** see `~/.claude/projects/.../memory/MEMORY.md` per [[claude-md-maintenance-rule-2026-09-06]]
