...
from src.ikigai.src.agents.v2.workers.investigation_dispatcher import (
...
def test_dispatch_once_archives_stale(tmp_queue):
    ...
    # Persist
    from src.mesh.investigation_queue import enqueue
    enqueue(stale)

    summary = dispatch_once()
    assert summary["archived_stale"] == 1
    # Verify it's now archived
    from src.mesh.investigation_queue import get
