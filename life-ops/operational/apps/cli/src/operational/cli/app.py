...
def tui(
    screen: str | None = typer.Option(
        None,
        "--screen", "-s",
        help="Jump directly to screen: dashboard | daily_flow | pomodoro_timer | habits | metrics | policy | journal",
    ),
    ...
    Screens: dashboard | daily_flow | pomodoro_timer | habits | metrics | policy | journal
    Keys: 1-7 switch screens | q quit | Ctrl+C interrupt
    """
    from operational.tui.app import PAVApp
    import asyncio

    app = PAVApp(initial_screen=screen, data_file=data_file, golden=golden)

    if debug:
        app._devtools = True  # type: ignore[attr-defined]
