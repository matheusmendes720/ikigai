...
def cmd_overview(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.overview", command="pav analytics overview") as ctx:
        ctx.info("data.loaded", tables=14)
        log = get_logger("analytics_cmd")
        with trace_command(log, "analytics.overview", command="pav analytics overview") as ctx:
            data = _load_all()
            ctx.info("data.loaded", tables=len(data))
            if json_out:
...
def cmd_qhe(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.qhe", command="pav analytics qhe") as ctx:
        data = _load_all()
        ctx.info("data.loaded", tables=len(data))
        if json_out:
...
def cmd_sleep(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.sleep", command="pav analytics sleep") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        if json_out:
            _json = json
...
def cmd_habits(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.habits", command="pav analytics habits") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        if json_out:
            _json = json
...
def cmd_pomodoro(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.pomodoro", command="pav analytics pomodoro") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        if json_out:
            _json = json
...
def cmd_policy(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.policy", command="pav analytics policy") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        if json_out:
            _json = json
...
def cmd_mood(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.mood", command="pav analytics mood") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        if json_out:
            _json = json
...
def cmd_week(
    ...
    """Week N digest (1-26)."""
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.week", command="pav analytics week") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        if json_out:
            _json = json
...
def cmd_report(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.report", command="pav analytics report") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
...
def cmd_quality(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.quality", command="pav analytics quality") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        if json_out:
            _json = json
...
def cmd_ritual(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.ritual", command="pav analytics ritual") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
...
def cmd_circadian(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.circadian", command="pav analytics circadian") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
...
def cmd_lunch(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.lunch", command="pav analytics lunch") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
...
def cmd_blocks(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.blocks", command="pav analytics blocks") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
...
def cmd_narrative(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.narrative", command="pav analytics narrative") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        week_list = weeks_in_range(_START, _END)
        if not (1 <= week_num <= len(week_list)):
...
def cmd_compare(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.compare", command="pav analytics compare") as ctx:
        ctx.info("data.loaded", tables=14)
        data = _load_all()
        week_list = weeks_in_range(_START, _END)
...
def cmd_growth(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.growth", command="pav analytics growth") as ctx:
        ctx.info("data.loaded", tables=14)
...
def cmd_trajectory(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.trajectory", command="pav analytics trajectory") as ctx:
        ctx.info("data.loaded", tables=14)
        ds = _load_dataset_from_csv_dir()
...
def cmd_forecast(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.forecast", command="pav analytics forecast") as ctx:
        ctx.info("data.loaded", tables=14)
        ds = _load_dataset_from_csv_dir()
...
def cmd_correlations(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.correlations", command="pav analytics correlations") as ctx:
        ctx.info("data.loaded", tables=14)
        ds = _load_dataset_from_csv_dir()
...
def cmd_insights(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.insights", command="pav analytics insights") as ctx:
        ctx.info("data.loaded", tables=14)
        ds = _load_dataset_from_csv_dir()
...
def cmd_scenarios(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.scenarios", command="pav analytics scenarios") as ctx:
        ctx.info("data.loaded", tables=14)
        ds = _load_dataset_from_csv_dir()
...
def cmd_all(
    ...
    log = get_logger("analytics_cmd")
    with trace_command(log, "analytics.all", command="pav analytics all") as ctx:
        ctx.info("data.loaded", tables=14)
        commands = [
