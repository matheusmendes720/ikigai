...
def create(
    ...
    with trace_command(log, "journal.create", command="journal create") as ctx:
        ...
        ctx.info("journal_entry.created", entry_id=entry.id, date=d.isoformat(), text_len=len(text))
        if json:
...
def list_entries(
    ...
    with trace_command(log, "journal.list", command="journal list") as ctx:
        items = journals.list()
        if target_date:
            d = date.fromisoformat(target_date)
            items = [j for j in items if j.date == d]

        ctx.info("journal.entries.listed", count=len(items), target_date=target_date)
        if json:
