...
def seed(
    ...
    with trace_command(log, "demo.seed", command="demo seed") as ctx:
        if json:
        ...
        ctx.info("demo.seeded", summary=summary)
...
def clear(
    ...
    with trace_command(log, "demo.clear", command="demo clear") as ctx:
        if json:
            msg = clear_demo_data()
...
def show(
    ...
    with trace_command(log, "demo.show", command="demo show") as ctx:
        stats = demo_stats()
        ctx.info("demo.stats", stats=stats)
        if json:
...
def week(
    ...
    with trace_command(log, "demo.week", command="demo week") as ctx:
        seed_demo_data()
        from operational.cli.commands.report_cmd import app as report_app
        args = ["weekly"]
        if json:
            args.append("--json")
        result = report_app(args=args, standalone_mode=False)
        ctx.info("demo.week.completed")
...
def export_csv(
    ...
    with trace_command(log, "demo.export_csv", command="demo export_csv", path=path) as ctx:
        ...
        ctx.info("demo.exported_csv", path=path, rows=written)
...
def import_csv(
    ...
    with trace_command(log, "demo.import_csv", command="demo import_csv", path=path, replace=replace) as ctx:
        from pathlib import Path
        ...
        ctx.info("demo.imported_csv", path=path, replace=replace, counts=counts)
        if json:
...
def dataset(
    ...
    """
    with trace_command(log, "demo.dataset", command="demo dataset", name=name) as ctx:
        import os
        ...
        ctx.info("demo.dataset.resolved", name=name)
